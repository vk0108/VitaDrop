...
export default function DonorAlertMapPage() {
  ...
  const handleSendNotification = async (donorId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/donors/notify`, {
        ...
        body: JSON.stringify({
          donor_id: donorId,
          message: 'Hospital : Shree\'s Hospital - Urgent blood donation needed! Take the eligibility test to confirm your availability.'
        })
      });
      // Only update to 'sent' or 'failed' if backend returns JSON with status
      if (response.ok) {
        ...
        } // else, keep as 'pending' until backend confirms
      }
      // If not ok, do not set to failed instantly; keep as pending
    } catch (error) {
  ...
  useEffect(() => {
    // --- Eligibility polling enabled ---
    const interval = setInterval(() => {
      Object.entries(notificationStatus).forEach(async ([donorId, status]) => {
        if (status === 'sent' || status === 'pending') {
          try {
            const eligibilityRes = await fetch('http://localhost:5000/api/hospital/get-eligibility-response', {
              method: 'POST',
